...
function transformToLis(obj) {
  ...
  for (let property in obj) {
    myArray.push(`<li>${property}: ${obj[property]}</li>`);
  }
  return myArray;
}
...
const totalSum = (input) => {
  ...
  input.map(element => element.map(element2 => sum = sum + element2));
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(element => element.filter(x => ((typeof x === 'number') && (x % 5 === 0))).map(y => Math.pow(2, y)));
};
...
let findShortest = (data) => {
  ...
  data.map(element => {
    if (count === 0) {
      shortint = Number(element.height);
      shortString = element.name;
    }
    else if (Number(element.height) < shortint) {
      shortint = Number(element.height);
      shortString = element.name;
    }
    count = count + 1;
