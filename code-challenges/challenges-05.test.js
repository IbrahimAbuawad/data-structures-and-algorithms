...
const toLastNames = people => {
  ...
  let newArray = [];
  people.map(element => {
    let init = element.firstName;
    let second = element.lastName;
    newArray.push(`${init} ${second}`);
  });
  return newArray;
};
...
const addPurchases = (arr) => {
  // Solution code here...
  let newArray = arr.map(element => element.purchasePrice);
  return newArray.reduce((init, second) => {
    return init + second;
  }, 0);
