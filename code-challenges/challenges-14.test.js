...
const detectTicTacToeWin = (board) => {
  ...
  //test verticals
  const verticals = [[board[0][0], board[1][0], board[2][0]], [board[0][1], board[1][1], board[2][1]], [board[0][2], board[1][2], board[2][2]]]
...
const testSet = (array) => {
  const regexX = /[xX]{1}/;
  const regexO = /[oO]{1}/;
  if (array.every(val => regexX.test(val))
    || array.every(val => regexO.test(val))) {
    return true;
  } else {
    return false;
  }
